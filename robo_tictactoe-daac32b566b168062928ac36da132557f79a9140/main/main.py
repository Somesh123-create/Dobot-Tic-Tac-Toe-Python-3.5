...
# define serial port here, usually /ttyACM0
SERIALPORT = glob.glob('/dev/tty[A-Za-z]*')
...
height = height of Tictactoe box in pixels
'''
...
def drawRegions(image):
    ...
    for i in range(regions.totalYintercepts-1):
        for ii in range(regions.totalXintercepts-1):
            ...
            # draw the rectangles
            cv2.rectangle(image,(0,0),(630,460),(0,255,0),2)
            fontIndex = fontIndex + 1
            # draw the labels
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(image,str(fontIndex),(0+5,0+25), font, 0.7,(255,255,255),2)
...
def drawOpponentMoves(image):
    ...
    for move in board.getSquares('X'):
        ...
        cv2.circle(image, (int(x), int(y)), 40, (255, 255, 0), 10)
...
def main():
    ...
    while not board.complete():
        ...
        if circles is not None:
            ...
            # ensures the arm won't move after the fourth turn
            if turn <= 3:
                # start the arm procedures
                arm.move(turn, computerMove - 1)
                turn = turn + 1
